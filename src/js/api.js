elife.factory('API', ['$rootScope', '$http', '$q', '$cookieStore', '$interval', '$timeout', function ($rootScope, $http, $q, $cookieStore, $interval, $timeout) {
    ...
    $rootScope.defaultImgUrl = "images/e生活缺省图.png";
...
//返回值没有res=0 没有data的情况
    ...
    api.isLogin = function () {
        return !!localStorage.getItem('t_k');
    };
    ...
    api.doLogout = function () {
        ...
        // window.location.hash = '/personal/index';
    };
    /*获取用户信息*/
    ...
    api.getYidaiBySearch = function (para) {
        para.t_k = $rootScope.token;
        para.c_no = $cookieStore.get('c_no');
        para.discount_role = "10000000000000000000";
        para.city_code = para.city_code || $cookieStore.get('city_code') || 110100;
        para.longitude = $cookieStore.get('longitude');
        para.latitude = $cookieStore.get('latitude');
        var promise = request(
...
//      baseUrl + '/OFSTCUST/storeisyd/getStoreIsydList.action',
    ...
    api.getJiFenBySearch = function (para) {
        para.t_k = $rootScope.token;
        para.c_no = $cookieStore.get('c_no');
        para.discount_role = "00010000000000000000";
        para.city_code = para.city_code || $cookieStore.get('city_code') || 110100;
        para.longitude = $cookieStore.get('longitude');
        para.latitude = $cookieStore.get('latitude');
        var promise = request(
...
//      baseUrl + '/OFSTCUST/storeisyd/getStoreIsydList.action',
    ...
    api.getShanKuBySearch = function (para) {
        para.t_k = $rootScope.token;
        para.c_no = $cookieStore.get('c_no');
        para.discount_role = "00000010000000000000";
        para.city_code = para.city_code || $cookieStore.get('city_code') || 110100;
        para.longitude = $cookieStore.get('longitude');
        para.latitude = $cookieStore.get('latitude');
        var promise = request(
...
//      baseUrl + '/OFSTCUST/storeisyd/getStoreIsydList.action',
    ...
    api.getFavorablekeyWord = function (para) {
        ...
        para.longitude = $cookieStore.get('longitude');
        var promise = request(
            baseUrl + '/OFSTCUST/iFavorablekeyWord/getFavorablekeyWord.action',
            'post',
            para
        );
        return promise;
...
// 商户详情工行卡优惠
    ...
    api.addComment = function (para) {
        ...
        para.t_k = $rootScope.token || $cookieStore.get('t_k');
        para.c_no = $cookieStore.get('c_no');
        console.log('未登录！！！');
        var promise = request(
...
//  };
    ...
    api.addOrder = function (para) {
        ...
        var promise = request(
            ...
            'http://192.168.2.99:8080/OFSTCUST/order/appOrder.action',
            'post',
            para
        );
        return promise;
    ...
    api.decryptQrcode = function (para) {
        ...
        var promise = request(
            ...
            'http://192.168.2.99:8080/OFSTCUST/order/qrCodeOrder.action',
            'post',
            para
        );
        return promise;
        ...
        return function () {
            ...
            api.getDailyGamble(para).then(function (data) {
                ...
                for (var index = 0; index < data.length; index++) {
                    var detail = {
                        labelState: data[index].status == '1' ? 'HOT' : '已结束',
    ...
    api.doGPSLocation = function (callback) {
        ...
        if (ICBCUtil.isElifeAndroid() || ICBCUtil.isAndroid()) {
            ...
            //do nothing
            // console.log('1');
            // document.getElementById('latitude').value = '30.26667';
            // document.getElementById('longitude').value = '120.20000';
            // var child = document.getElementById('trigger');
            // child.focus();
            // child.blur();
        ...
        if (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                for (var z = 0; z < data[i].discount_role.length; z++) {
                    if (data[i].discount_role[z] === ' icon_yi') {
                        data[i].new_discount_role.push(' icon_yi');
                    }
                    if (data[i].discount_role[z] === ' icon_fen') {
                        data[i].new_discount_role.push(' icon_fen');
                    }
                    if (data[i].discount_role[z] === ' icon_shan') {
                        data[i].new_discount_role.push(' icon_shan');
                    }
                    if (data[i].discount_role[z] === ' icon_ji') {
                        data[i].new_discount_role.push(' icon_ji');
                    }
                }
                var stars = [];
                for (var j = 0; j < 5; j++) {
