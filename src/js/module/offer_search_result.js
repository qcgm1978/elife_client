elife.controller('OfferSearchResultCtrl', [
    ...
    '$cookieStore',
    'API',
    function ($scope, $routeParams, SharedState, $http, $cookieStore, API) {
            ...
            if (!$scope.search_type) {
                ...
                API.getFavorablekeyWord(para).then(function (data) {
                    ...
                    if ((!$scope.offerList || $scope.offerList.length === 0)) {
                        ...
                        for (var i = para.s_row; i < $scope.offerList.length; i++) {
                            ...
                            for (var j = 0; j < $scope.offerList[i].allList.length; j++) {
                                $scope.offerList[i].allList[j].detailPath = $scope.offerList[i].allList[j].txt_pft_title !== undefined ? "#/discount/business_offer/" + $scope.offerList[i].allList[j].store_code + '/' + $scope.offerList[i].allList[j].text_code + '/store' : "#/discount/customers/" + $scope.offerList[i].allList[j].gpp_code;
                            }
                        }
                    }
                    $scope.loading = false;
                ...
                API.getSelectKeywordDiscount(para).then(function (data) {
                    ...
                    if ((!$scope.offerList || $scope.offerList.length === 0)) {
                        ...
                        for (var i = para.s_row; i < $scope.offerList.length; i++) {
                            ...
                            for (var j = 0; j < $scope.offerList[i].allList.length; j++) {
                                if ($scope.offerList[i].allList[j].txt_pft_title !== undefined) {
                                ...
                                } else if ($scope.offerList[i].allList[j].gpp_code !== undefined) {
                                    $scope.offerList[i].allList[j].detailPath = "#/discount/customers/" + $scope.offerList[i].allList[j].gpp_code;
                                } else if ($scope.offerList[i].allList[j].bank_pft_code !== undefined) {
...
//排序
    // $scope.filterBySort = function(sortType,sortName){
    //   $scope.currentSortType = sortType;
    //    $scope.currentSortName = sortName;
    //    $cookieStore.remove(KEY_STORE_ORDER_KEY);
    //   $cookieStore.remove(KEY_STORE_ORDER_NAME);
    //   $cookieStore.put(KEY_STORE_ORDER_KEY, $scope.currentSortType);
    //   $cookieStore.put(KEY_STORE_ORDER_NAME, $scope.currentSortName);
    //   $scope.getList(0);
    //   $scope.filterToggle(0);
    // };
    $scope.filterByRank = function (rType, rName) {
