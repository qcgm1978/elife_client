elife.controller('IndexCtrl', ['$scope', '$rootScope', '$cookieStore', '$http', 'API', function ($scope, $rootScope, $cookieStore, $http, API) {
    // 初始化参数
    console.log(1111111+2222222222);
        ...
        if (data) {
            for (var i = 0; i < $scope.guessLike.length; i++) {
                ...
                for (var z = 0; z < $scope.guessLike[i].discount_role.length; z++) {
                    if ($scope.guessLike[i].discount_role[z] === ' icon_yi') {
                        $scope.guessLike[i].new_discount_role.push(' icon_yi');
                    }
                    if ($scope.guessLike[i].discount_role[z] === ' icon_fen') {
                        $scope.guessLike[i].new_discount_role.push(' icon_fen');
                    }
                    if ($scope.guessLike[i].discount_role[z] === ' icon_shan') {
                        $scope.guessLike[i].new_discount_role.push(' icon_shan');
                    }
                    if ($scope.guessLike[i].discount_role[z] === ' icon_ji') {
                        $scope.guessLike[i].new_discount_role.push(' icon_ji');
                    }
                }
                console.log($scope.guessLike[i].new_discount_role);
                var stars = [];
                for (var j = 0; j < 5; j++) {
