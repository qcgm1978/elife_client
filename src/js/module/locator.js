//定位选择页面js
elife.controller('LocatorCtrl', ['$scope','$http', 'API', '$anchorScroll','$cookieStore','$timeout','$rootScope','_serarchCity', function($scope,$http, API, $anchorScroll,$cookieStore,$timeout,$rootScope,_serarchCity){
  ...
  var setHistoryCity = function(cityName, cityCode){
    var old = "";
    if($cookieStore.get('historyCity')) {
      old = $cookieStore.get('historyCity');
    }
    var oldCitys = [];
    if(old !== "")
      oldCitys = old.split(",");
    ...
    $cookieStore.put('historyCity',now);
    getHistoryCity();
  };
  //个人信息
  ...
  function getHistoryCity() {
    var hcity = [];
    if($cookieStore.get('historyCity')){
       hcity = $cookieStore.get('historyCity').split(',');
    }
    var historyCity = [];
