elife.controller('BusinessInfoCtrl', ['$scope', '$http', '$cookieStore', '$routeParams', 'SharedState', 'API', '$rootScope', '$timeout', '$location',
    function ($scope, $http, $cookieStore, $routeParams, SharedState, API, $rootScope, $timeout, $location) {
        ...
        $scope.mark2 = false;
// //获取商户信息的工行卡优惠
//  API.getBusinessInfoICBC({'store_code': $scope.id}).then(function(data){
//     console.log('商户ICBC');
//     console.log(data);
//     var favList = data;
//     $scope.favStore = favList;
//     $scope.busiInfo = {
//   }, function(data){
...
//   });
        //获取商户信息的工行卡优惠
// $scope.getIcbcList = function(){
//   var para = {
//     "store_code" : $scope.id
//   };
//    para.s_row = 1;
//    para.e_row = 10;
//    para.order_key='distance';
...
//    API.getBusinessInfoICBC(para).then(function(data){
//     $scope.icbcList = data;
//     console.log('商户ICBC');
//     console.log(data);
//   }, function(data){
               ...
               $scope.head_code=data.parent_store_code;
                ...
                for (var i = 0; i < $scope.busiInfo.flags.length; i++) {
                    if ($scope.busiInfo.flags[i] === ' icon_yi') {
                        $scope.busiInfo.new_flags.push(' icon_yi');
                    }
                    if ($scope.busiInfo.flags[i] === ' icon_fen') {
                        $scope.busiInfo.new_flags.push(' icon_fen');
                    }
                    if ($scope.busiInfo.flags[i] === ' icon_shan') {
                        $scope.busiInfo.new_flags.push(' icon_shan');
                    }
                    if ($scope.busiInfo.flags[i] === ' icon_ji') {
                        $scope.busiInfo.new_flags.push(' icon_ji');
                    }
                }
                console.log($scope.busiInfo.array);
            },
            function (data) {
...
//网友点评
        API.getComments({
            ...
            $scope.toast("请检查网络状况2");
        });
//测试api
        // $http.post('http://223.223.177.32:8082/OFSTCUST/storequery/storeDetail.action',{
        // 'store_code' : '2001100001'
        // })
        ...
        //   }
        // });
// $http.post('http://223.223.177.32:8082/OFSTCUST/cuinfo/findCuMoreById.action ',{
//   't_k' : $rootScope.token,
...
//    }
            ...
            API.getStoreGpp({
                ...
                $scope.storeGppList = data.data;
            }, function (data) {
                $scope.toast('请检查网络设置4');
...
//   };
        $scope.displayIconInfo = function (flag) {
            ...
            switch (flag) {
                case ' icon_yi':
                    prompt = '商户支持消费贷款（满100元）';
                    break;
                case ' icon_fen':
                    prompt = '商户支持分期付款（满600元）';
                    break;
                case ' icon_ji':
                    prompt = '商户支持工行卡积分当钱花 ';
                    break;
