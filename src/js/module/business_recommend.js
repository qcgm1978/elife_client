elife.controller('businessRecommendCtrl', ['$scope','$routeParams','SharedState','$http','$cookieStore','API', function ($scope,$routeParams, SharedState,$http,$cookieStore,API){
       ...
       if ((!$scope.list || $scope.list.length === 0)){
        ...
        // 解析商户图标
                for (var i = 0; i < $scope.list.length; i++) {
                    var stars = [];
                    for(var j=0;j<5;j++) {
                        if (j+1<=$scope.list[i].dcmt_level) {
                            stars[j] = {"type":"full"};
                        } else if(j - $scope.list[i].dcmt_level < 0) {
                            stars[j] = {"type":"half"};
                        }else {
                            stars[j] = {"type" : "gray"};
                        }
